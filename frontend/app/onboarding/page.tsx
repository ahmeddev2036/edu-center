...
export default function OnboardingPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!tRes.ok) {
        ...
        throw new Error(d.message || 'فشل إنشاء الحساب');
      }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: 480, padding: 40 }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#1e1b4b', marginBottom: 12 }}>مرحباً بك في EduFlow!</h1>
          <p style={{ color: '#6b7280', marginBottom: 32 }}>تم إنشاء حسابك بنجاح. لديك 14 يوم تجريبي مجاني.</p>
          <button
            onClick={() => router.push('/dashboard')}
            style={{ background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 12, padding: '14px 40px', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}
          >
            ابدأ الاستخدام →
          </button>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e1b4b 0%, #4f46e5 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: 40, width: '100%', maxWidth: 520 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ fontSize: 40, marginBottom: 8 }}>🎓</div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1e1b4b' }}>إنشاء حساب جديد</h1>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gap: 16 }}>
          <div>
            ...
            <input
              style={{ width: '100%', padding: '10px 14px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 15, boxSizing: 'border-box' }}
              value={form.centerName}
              onChange={e => setForm({ ...form, centerName: e.target.value, slug: slugify(e.target.value) })}
              placeholder="مثال: سنتر النجاح"
              required
            />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 600, fontSize: 14 }}>رابط السنتر (slug) *</label>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 600, fontSize: 14 }}>البريد الإلكتروني *</label>
            <input
              style={{ width: '100%', padding: '10px 14px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 15, boxSizing: 'border-box' }}
              type="email"
          ...
          <div>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 600, fontSize: 14 }}>كلمة المرور *</label>
            <input
              style={{ width: '100%', padding: '10px 14px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 15, boxSizing: 'border-box' }}
              type="password"
              value={form.password}
          ...
          <div>
            <label style={{ display: 'block', marginBottom: 6, fontWeight: 600, fontSize: 14 }}>الخطة</label>
            <select
              style={{ width: '100%', padding: '10px 14px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 15 }}
              value={form.plan}
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 10, padding: '14px', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}
          >
