      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
        <div className="card">
          ...
          <form onSubmit={handleGenerate}>
            <div className="form-group">
              ...
              <input className="input" value={form.studentName} onChange={e => setForm({ ...form, studentName: e.target.value })} placeholder="أو اختر من القائمة أدناه" required />
              {students.length > 0 && (
                <select className="input" style={{ marginTop: 8 }} onChange={e => setForm({ ...form, studentName: e.target.value })} value="">
                  <option value="">اختر طالب...</option>
                  {students.map((s: any) => <option key={s.id} value={s.fullName}>{s.fullName}</option>)}
                </select>
              )}
            </div>
            <div className="form-group">
              <label>عنوان الامتحان *</label>
              <input className="input" value={form.examTitle} onChange={e => setForm({ ...form, examTitle: e.target.value })} placeholder="أو اختر من القائمة أدناه" required />
              {exams.length > 0 && (
                <select className="input" style={{ marginTop: 8 }} onChange={e => setForm({ ...form, examTitle: e.target.value })} value="">
                  <option value="">اختر امتحان...</option>
                  {exams.map((ex: any) => <option key={ex.id} value={ex.title}>{ex.title}</option>)}
                </select>
              )}
            </div>
            <div className="form-group">
        ...
        <div className="card">
          ...
          <div style={{ marginTop: 24, padding: 16, background: '#f0f9ff', borderRadius: 8, border: '1px solid #bae6fd' }}>
            <div style={{ fontSize: 13, color: '#0369a1', fontWeight: 600, marginBottom: 8 }}>نظام التقديرات:</div>
            <div style={{ fontSize: 13, color: '#0369a1', lineHeight: 2 }}>
