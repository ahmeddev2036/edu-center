  ...
  return (
    <div>
      <div className="page-header">
        <h1>الإشعارات</h1>
        <button className="btn-primary" onClick={() => setShowModal(true)}>+ إرسال إشعار</button>
      </div>
      ...
      <div className="card">
        ...
        {logs.length === 0 ? (
          ...
          <table>
            <thead>
              <tr><th>القناة</th><th>المستلم</th><th>القالب</th><th>الحالة</th><th>التاريخ</th></tr>
            </thead>
            <tbody>
              {logs.map((l: any) => (
                <tr key={l.id}>
                  ...
                  <td>{l.recipient}</td>
                  <td style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.template}</td>
                  <td>
                    <span className={`badge ${statusBadge[l.status] || 'badge-yellow'}`}>
                      {l.status === 'queued' ? 'في الانتظار' : l.status === 'sent' ? 'مُرسَل' : 'فشل'}
                    </span>
                  </td>
