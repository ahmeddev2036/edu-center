...
export default function StudentsPage() {
  ...
  useEffect(() => { load(); }, []);
  async function load() { try { setStudents(await api.getStudents()); } catch {} }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      editId ? await api.updateStudent(editId, form) : await api.createStudent(form);
  ...
  return (
    <div>
      ...
      <div className="card">
        <input placeholder="بحث بالاسم أو الكود أو المجموعة..." value={search}
          onChange={e => setSearch(e.target.value)} style={{ marginBottom: 14 }} />
        {filtered.length === 0 ? (
          <p style={{ color: '#9ca3af', textAlign: 'center', padding: '24px 0' }}>لا يوجد طلاب</p>
        ) : (
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>الاسم</th><th>الكود</th><th>المجموعة</th><th>الهاتف</th><th>إجراءات</th></tr>
              </thead>
              <tbody>
                {filtered.map((s: any) => (
                  <tr key={s.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button className="btn-ghost" style={{ padding: '5px 10px', fontSize: 12 }} onClick={() => openEdit(s)}>تعديل</button>
                        <button className="btn-danger" style={{ padding: '5px 10px', fontSize: 12 }} onClick={() => handleDelete(s.id)}>حذف</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group"><label>الكود / الباركود *</label>
                <input value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} required /></div>
              <div className="form-group"><label>المجموعة</label>
                <input value={form.groupName} onChange={e => setForm({ ...form, groupName: e.target.value })} /></div>
              <div className="form-group"><label>هاتف ولي الأمر</label>
                <input value={form.guardianPhone} onChange={e => setForm({ ...form, guardianPhone: e.target.value })} /></div>
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button type="submit" className="btn-primary" disabled={loading} style={{ flex: 1 }}>
                  {loading ? 'جاري الحفظ...' : editId ? 'تحديث' : 'حفظ'}</button>
